...
export const CONFIG_FIELDS: readonly ConfigField[] = [
  ...
  { key: "AI_FALLBACKS", group: "AI rephrasing (OpenAI-compatible)", label: "Fallback providers (JSON)", secret: true, help: "Optional JSON array of backup providers, tried in order whenever the one before it fails for ANY reason (error, outage, rate-limit, empty response). Each entry: {\"apiKey\":\"…\",\"model\":\"…\",\"baseUrl\":\"…\"} — baseUrl optional (defaults to OpenAI). Example: [{\"apiKey\":\"gsk_…\",\"baseUrl\":\"https://api.groq.com/openai/v1\",\"model\":\"llama-3.3-70b-versatile\"}]" },
