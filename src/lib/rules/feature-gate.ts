...
/** The PRO features a rule with this response would use (empty for a free rule). */
export function requiredRuleFeatures(responseType: string, responseConfig: Record<string, unknown>): Feature[] {
  const feats: Feature[] = [];
...
export async function firstUnlicensedRuleFeature(
  after: { responseType: string; responseConfig: Record<string, unknown> },
  before?: { responseType: string; responseConfig: Record<string, unknown> },
): Promise<Feature | null> {
  const grandfathered = before
    ? new Set(requiredRuleFeatures(before.responseType, before.responseConfig))
    : new Set<Feature>();
  for (const f of requiredRuleFeatures(after.responseType, after.responseConfig)) {
