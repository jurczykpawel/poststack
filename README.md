...
| Webhook "Test" works, real messages don't | The app is in **Development** mode — switch it to **Live**. A *test app* (a "test version" of another app) is always in Development mode and cannot go Live; use a regular app. |
...
| `META_PROBE_IG_USER_ID` | *Optional.* IGSID of a dedicated test recipient with an open messaging window. Without it the real IG DM send is skipped and must be checked manually (see below). |
...
sends one Facebook test DM (plus one Instagram test DM when `META_PROBE_IG_USER_ID` is set).

It also sends the production Instagram DM payload to a recipient that cannot exist. The expected
"recipient not found" rejection proves the endpoint, version and payload keys still work, and is
reported as **PARTIAL**: Meta checks the recipient before required fields, so a newly required field,
a changed success response or delivery problems are *not* covered. That is why the generated PR
carries a mandatory checklist item — send one real Instagram DM from TEST before merging. The PR also
lists the changelog sections for the new version that mention surfaces PostStack calls (a keyword
scan to speed up review, not a compatibility guarantee). Before dispatching it, open fresh conversations
from dedicated test accounts to the Page (and the Instagram account, if you set `META_PROBE_IG_USER_ID`) so the replies are policy-compliant.
